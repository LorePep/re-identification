...
from datasets import SingleImageDataset, TripletNetworkDataset
...
def get_dataloaders(
    ...
    sampler=None,
    img_size=224,
    mean=DEFAULT_RESNET_MEAN,
    std=DEFAULT_RESNET_STD,
    batch_size=16,
):
    labels_to_idx = {
        label: i for i, label in enumerate(sorted(df_train["Id"].unique()))
    }
    ...
    train_dataset = TripletNetworkDataset(
        path=train_path, df=df_train, transforms=train_tranforms
    )
    ...
    if sampler:
        train_dataloader = DataLoader(
            single_train_dataset, num_workers=4, batch_sampler=sampler
        )
    else:
        train_dataloader = DataLoader(
    ...
    single_train_dataloader = DataLoader(
        single_train_dataset, shuffle=False, num_workers=4, batch_size=batch_size
    )

    single_val_dataloader = DataLoader(
        single_val_dataset, shuffle=False, num_workers=4, batch_size=batch_size
    )
