...
from triplet_selectors import HardBatchTripletSelector, pairwise_distance
...
def train(train_path, labels, boxes, output_dir, num_epochs, hard, verbose):
    ...
    if verbose:
        logging.info(
            "Train size: {}, validation size: {}".format(len(df_train), len(df_val))
        )
...
def _train(
    ...
    for epoch in range(num_epochs):
        ...
        for data in tqdm(train_dataloader):
            ...
            dists = pairwise_distance(embeddings, embeddings).detach().cpu().numpy()
        ...
        if patience_counter == patience:
            ...
            return (
                model,
                dict(
                    emb_norm_p50=history_emb_norm_50,
                    emb_norm_p95=history_emb_norm_95,
                    emb_dist_p50=history_emb_dist_50,
                    emb_dist_p95=history_emb_dist_95,
                ),
            )
    ...
    return (
        model,
        dict(
            history_emb_norm_50=history_emb_norm_50,
            history_emb_norm_95=history_emb_norm_95,
            history_emb_dist_50=history_emb_dist_50,
            history_emb_dist_95=history_emb_dist_95,
        ),
    )
...
def _plot_history(history):
    ...
    plt.subplot(211)
    plt.plot(history["emb_norm_p50"], label="p50")
    plt.plot(history["emb_norm_p95"], label="P95")
    plt.xlabel("batches")
    plt.title("L2 Norm of embeddings")
    plt.legend()
    plt.subplot(212)
    plt.plot(history["emb_dist_p50"], label="p50")
    plt.plot(history["emb_dist_p95"], label="P95")
...
def _plot_embeddings(embeddings, targets, idxs):
    ...
    for idx in idxs:
        ...
        plt.scatter(
            ...
            embeddings[inds, 1],
            alpha=0.5,
            color=COLOR_PALETTE[int(idx) % 10],
        )
    plt.legend(idxs)
